/*
    Name: Bilal Malik
    Program: COMP 3300 Assignment 1 Program 3
    Date: 13/02/2020
 */

//compile with gcc -pthread assign3_program3.c -lm
...
void *runner(void *param)
{
    ...
    for(i = 0; i < POINTS; i++)
    {
       ...
       /* to check is (x,y) point within the circle by seeing if less then size of circle radius*/
        ...
        pthread_mutex_lock(&mutex);//lock so only this thread can increment
        circle_count += hit_count;
        pthread_mutex_unlock(&mutex);// unlock, this removes the race condition
        pthread_exit(0);
}
